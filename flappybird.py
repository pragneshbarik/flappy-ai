...
global_settings = {
    "game_fps": 120,
    "gravity": 0.6,
    "bird_initial_pos": (75, H/2),  # (x, y)
    "bird_initial_vel": 0,
    "bird_max_vel": -9.0,
    "bird_initial_angle": 0,
    "bird_jump_lift": 9.0,  # +ve for upwards
    "bird_rotational_vel": 2.5,
    "pipe_initial_pos_x": 200,
    "pipe_velocity": 5,  #
    "pipe_ygap": 125,
    ...
    def update(self, game_state):
        ...
        if (self.game_state):
            ...
            if (len(self.pipes) < self.n_pipes):
                new_pipe = Pipe(
                    self.pipes[len(self.pipes)-1].x + 200, self.game_params)
...
class Environment:
    ...
    def network_parameters(self, bird):
        valid_pipes = [
            pipe for pipe in self.pipes.pipes if pipe.up.rect.right > self.bird.rect.left]
        return ((valid_pipes[0].up.rect.bottom - bird.rect.top), (valid_pipes[0].down.rect.top-bird.rect.bottom), bird.rect.centery, bird.velocity, abs(valid_pipes[0].up.rect.left - bird.rect.right))
    ...
    def evaluate_genomes(self, genomes, config):
        ...
        while not done and len(self.birds) > 0:
            ...
            score = score_font.render(
                f"SCORE: {self.pipes.passed}", True, (0, 0, 0))
            gen = score_font.render(
                f"GENERATION: {self.generations}", True, (0, 0, 0))
            ...
            for id, bird in enumerate(self.birds):
                ...
                output = self.networks[id].activate(
                    self.network_parameters(bird))[0]
            ...
            self.bg.ground.draw(self.screen)
            self.screen.blit(score, (5, H-30))
            self.screen.blit(gen, (5, H-55))
    ...
    def play(self):
        ...
        while True:
            ...
            self.screen.blit(score, (W-100, 0))
            pygame.display.update()
            clock.tick(self.fps)
